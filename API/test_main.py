...
def path_deploy_normal(old_path, new_path, flow, state_cur, prt, in_port, out_port, clock, bd, if_delay):
    bdid = bd
    clk = clock + 1

    rule_set = rule_construct(old_path, new_path, flow, state_cur, prt, out_port, clk)
    state_next = state_update(rule_set, state_cur, clk)
    ...
    #rule_set_reverse = setTMP(old_path_reverse, new_path_reverse, flow_reverse, state_next_next, state_next_next, rule_set_reverse, clk)
    #state_next_next = state_update(rule_set_reverse, state_next, clk)
    #state_next.copy_state(state_cur)
    ...
    #for i in range(1):
        ...
        if cmp(flow_list[y], path_list['old_path'][i]) == 0:
            ...
            if proto:
                ...
                deploy_ret = path_deploy_normal(old_path, new_path, flow, state_cur, priority, in_port, out_port, clk, bdid, 0)
            ...
            if 'packets received' in ping_ret:
                ...
                retpath = '/home/shengliu/Workspace/mininet/haha/API/ping_result.txt'
                fp = open(retpath, 'a+')
...
def test_run(K, fat_tree_net, pkt_rate, proto, nt):
    ...
    if proto:
        deploy_ret = path_deploy(old_path, new_path, flow, state_cur, priority, in_port, out_port, clk, bdid, 0)
    else:
        deploy_ret = path_deploy_normal(old_path, new_path, flow, state_cur, priority, in_port, out_port, clk, bdid, 1)
    ...
    if 'packets received' not in ping_ret:
        return False
    ...
    retpath = '/home/shengliu/Workspace/mininet/haha/API/ping_result.txt'
    fp = open(retpath, 'a+')
    fp.write('pkt_rate: %d sent_num: %s recv_num: %s\n' %(pkt_rate, sent_num, recv_num))
    fp.close()
