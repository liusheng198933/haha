...
class FatTree(Topo):
    "Fat tree topology with k number."
    ...
    def __init__(self, k):
        ...
        total_host = k*k
        total_switch = k*k + (k/2)*(k/2)

        host_list = []
        for i in range(total_host):
            host_list.append(self.addHost('h%i' %(i+1), ip='10.0.1.%i' %i))

        switch_list = []
        for i in range(total_switch+1):
            switch_list.append(self.addSwitch('s%i' %(i+1), protocols='OpenFlow14'))


        for i in range(total_host):
            self.addLink(host_list[i], switch_list[i/2], 0, i%2)
